...
weeks = {"Mon":u"星期一", "Tue":u"星期二", "Wed":u"星期三", "Thu":u"星期四", "Fri":u"星期五", "Sat":u"星期六", "Sun":u"星期日"}
months = {"Jan":u"一月", "Feb":u"二月", "Mar":u"三月", "Apr":u"四月", "May":u"五月", "Jun":u"六月", "Jul":u"七月", "Aug":u"八月", "Sep":u"九月", "Oct":u"十月", "Nov":u"十一月", "Dec":u"十二月"}
...
class MyFrame(wx.Frame):
    def __init__(self, parent, id, title):        
        ...
        self.Centre()
        self.Show(True)
        self.show_company_stock_info()
        
    def show_company_stock_info(self):        
        realtime_stock = RealtimeStock(self.stock_num1)
        dic_stock = realtime_stock.real
        strLabel = dic_stock.get('name') + '   '
        strLabel += dic_stock.get('c') + '   '
        strLabel += dic_stock.get('range') + '   '
        strLabel += 'H:' + dic_stock.get('h') + '   '
        strLabel += 'L:' + dic_stock.get('l') + '   '
        strLabel += dic_stock.get('time') + '   '
        moment = asctime()        
        strLabel +=  unicode(moment[11:19])
        self.time1.SetLabel(strLabel) 
        wx.CallLater(30000, self.show_company_stock_info)
                     
    def ButtonClick(self, event):
        name = self.name.GetValue()        
        message = u"查詢 " + unicode(name)
        self.message.SetLabel(message)
        self.stock_num1 = str(name)        

def show_company_stock_info_by_timeDelay(stock_number, delay , times, StaticTextNum):             
    show_company_stock_info(str(stock_number), StaticTextNum)
